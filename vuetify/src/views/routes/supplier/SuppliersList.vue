          ...
          <tr v-on:click="onClick(item)">
            ...
            <td>
              <div
                ...
                class="body-1"
                v-text="getMachiens(item.spare_parts)"
              />
              <div v-else>
                <v-icon
                  large
...
<script>
  ...
  import QrScanner from '@/views/routes/supplier/SupplierQRScanner'
  export default {
    name: 'SuppliersList',
    ...
    async mounted () {
      ...
      this.loading = true;
      this.data = await this.getSuppliersList();
    ...
    methods: {
      ...
      remove () {
        return this.$axios({
          ...
          data: {
            query: `mutation {
            deleteSupplier(input:{where: {id: "${this.currentId}"} }) {
              supplier {
                id
              }
            }
        ...
        `,
          ...
          .then(async ({ data }) => {
            this.snackbar = {
              message: 'A Supplier has been successfully removed.',
              type: 'success',
              snackbar: true,
            };
            // Refetch Sensors
      ...
      onAddClick () {
        this.$router.push('/suppliers/supplier-form')
      },
      onEditClick (action, id) {
        this.$router.push({ name: 'SupplierFormAction', params: { action, id } })
      },
      onClick(item) {
        if (!this.isAdmin) {
          this.$router.push({ name: 'SupplierFormAction', params: { action: 'modify', id: item.id } });
        }
      },
      ...
      async getSuppliersList (payload) {
        const url = '/graphql';
        return this.$axios({
          method: 'POST',
          url: url,
          ...
          const suppliers = res.data.data.suppliers;
          // Get data count
          this.count = await this.$axios({
            ...
            data: {
              query: `{
                suppliersConnection(where: {company: "${this.company}"}){
                  aggregate{
                    count
                  }
                }
            ...
            if (res.data.data.suppliersConnection) {
              return res.data.data.suppliersConnection.aggregate.count
            } else {
              return 0
            }
          });
      ...
      async getSparePartFilter () {
        ...
        return this.$axios({
          ...
          data: {
            query: `{
                  suppliers (where: {company: "${this.company}"}, limit: ${limit || 10}, sort: "${sortBy || 'name'}:${sortDesc ? 'desc' : 'asc'}", start: ${start || 0})
                  {
                    id
                    name
                    spare_parts {
