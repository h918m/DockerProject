        ...
        >
          ...
          <v-col>
            <div class="font-weight-bold grey--text">
              Scan QR Code attached to the Supplier
            </div>
            <!-- Built-in scanner -->
            <qrcode-stream
              @decode="onDecode"
      ...
      onDecode (result) {
        if (result) {
          ...
          return this.$axios({
            ...
            this.$router.push({ name: 'SupplierFormAction',params: { action: 'modify', id: data.data.supplier.id } })
          }).catch(() => (this.error = 'QR code does not match any Supplier'))
        } else {
