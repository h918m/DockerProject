...
<script>
  ...
  import QrScanner from '@/views/routes/machine/MachineQRScanner'
    ...
    methods: {
      ...
      remove() {
        return this.$axios({
          ...
          data: {
            query: `mutation {
              deleteMachine(input:{where: {id: "${this.currentId}"} }) {
                machine{
                  name
                }
              }
          ...
          .then(async ({data}) => {
            this.snackbar = {
              message: 'Machine has been successfully removed.',
              type: 'success',
              snackbar: true,
            };
            // Refetch Sensors
      ...
      async getMachinesData(payload) {
        ...
        return this.$axios({
          ...
          this.count = await this.$axios({
            ...
            data: {
              query: `{
                machinesConnection(where: {company: "${this.company}"}){
                  aggregate{
                    count
                  }
                }
          ...
          }).then((res) => res.data.data.machinesConnection.aggregate.count);
          return machines
        }).catch(err => {
          return [];
        })
      ...
      async getMachineReadings() {
        ...
        return this.$axios({
          ...
          data: {
            query: `{
                machines(where: {company: "${this.company}"}, limit: ${limit || 10}, sort: "${sortBy || 'name'}:${sortDesc ? 'desc' : 'asc'}", start: ${start || 0}){
                  id
                  name
                  QR
                  image{
