        ...
        >
          ...
          <v-col>
            <div class="font-weight-bold grey--text">
              Scan QR Code attached to the Spare part
            </div>
            <!-- Built-in scanner -->
            <qrcode-stream
              @decode="onDecode"
      ...
      onDecode (result) {
        if (result) {
          ...
          return this.$axios({
            ...
            this.$router.push({ name: 'SparePartFormAction',params: { action: 'modify', id: data.data.sparePart.id } })
          }).catch(() => (this.error = 'QR code does not match any Spare parts'))
        } else {
