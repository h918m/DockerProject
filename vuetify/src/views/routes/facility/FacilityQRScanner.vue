        ...
        >
          ...
          <v-col>
            <div class="font-weight-bold grey--text">
              Scan QR Code attached to the Facility
            </div>
            <!-- Built-in scanner -->
            <qrcode-stream
              @decode="onDecode"
...
<script>
  export default {
    name: 'FacilityFacilityQRScanner',
    data: function () {
      return {
        result: '',
        error: '',
      ...
      onDecode (result) {
        if (result) {
          ...
          return this.$axios({
            ...
            this.$router.push({ name: 'FacilityFormAction',params: { action: 'modify', id: data.data.facility.id } })
          }).catch(() => (this.error = 'QR code does not match any Facility'))
        } else {
