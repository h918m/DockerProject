        ...
        >
          ...
          <v-col>
            <div class="font-weight-bold grey--text">
              Scan QR Code attached to the Equipment
            </div>
            <!-- Built-in scanner -->
            <qrcode-stream
              @decode="onDecode"
        ...
        onDecode (result) {
            if (result) {
                ...
                return this.$axios({
                  ...
                  data: {
                    query: `{
                        purchaseOrder(id:"${result}"){
                          id
                        }
                      }`,
                  },
                }).then(({ data }) => {
                  this.$router.push({ name: 'PurchaseOrderFormAction', params: { id: data.data.machine.id, action: 'modify' } })
                }).catch(() => (this.error = 'QR code does not match any Equipment'))
            } else {
