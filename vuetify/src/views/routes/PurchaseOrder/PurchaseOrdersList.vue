<template>
  ...
  >
    ...
    <qr-scanner/>
    <base-v-component
      heading="List of PurchaseOrders"
...
<script>
  ...
  import QrScanner from '@/views/routes/PurchaseOrder/PurchaseOrderQRScanner'
  ...
  export default {
    ...
    methods: {
      onAddClick() {
        this.$router.push('/purchase-orders/purchaseorder-form')
      },
      getNames(aray) {
        let name = "";
        if (aray.length === 0) {
      ...
      remove() {
        return this.$axios({
          ...
          data: {
            query: `mutation {
              deletePurchaseOrder(input:{where: {id: "${this.currentId}"} }) {
                purchaseOrder{
                  name
                }
              }
      ...
      openHistoricalData(id) {
        this.$router.push({name: 'PurchaseOrderFormAction', params: {action: 'modify',id: id}})
      },
      onClick(item) {
        if (!this.isAdmin) {
          this.$router.push({ name: 'PurchaseOrderFormAction', params: {action: 'modify', id: item.id}});
        }
      },
      async getMachinesData(payload) {
        ...
        return this.$axios({
          ...
          data: {
            query: `
              {
                purchaseOrders(where: {company: "${this.company}"}){
                  id
                  name
                  QR
                  image{
          ...
          this.count = await this.$axios({
            ...
            data: {
              query: `{
                purchaseOrdersConnection(where: {company: "${this.company}"}){
                  aggregate{
                    count
                  }
                }
          ...
          }).then((res) => res.data.data.purchaseOrdersConnection.aggregate.count);
          return machines
        }).catch(err => {
          return [];
        })
      ...
      async getMachineReadings() {
        ...
        return this.$axios({
          ...
          data: {
            query: `{
                purchaseOrders(where: {company: "${this.company}"}, limit: ${limit || 10}, sort: "${sortBy || 'name'}:${sortDesc ? 'desc' : 'asc'}", start: ${start || 0}){
                  id
                  name
                  QR
                  image{
