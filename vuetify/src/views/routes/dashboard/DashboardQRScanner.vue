        ...
        >
          ...
          <v-col>
            <div class="font-weight-bold grey--text">
              Scan QR Code attached to the Machine
            </div>
            <!-- Built-in scanner -->
            <qrcode-stream
              @decode="onDecode"
      ...
      onDecode (result) {
        if (result) {
          ...
          return this.$axios({
            ...
            // this.$router.push({ name: 'MachineSubMachinesForMachine', params: { id: data.data.machine.id } })
            this.handleResult(data.data.machine);
